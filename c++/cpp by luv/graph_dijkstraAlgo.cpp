...
// https://cp-algorithms.com/graph/dijks... (Scroll down on link for questions)
...
vector<pair<int, int>> g[N]; // as it is weighted graph
...
int main()
{
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        cin >> x >> y >> wt;
        g[x].push_back({y, wt});
        // g[x].push_back({y,wt}); if not directed then
    }
